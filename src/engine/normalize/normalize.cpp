...
namespace engine {
bool isSizeMatch(size_t len) { return len == protocol::kWireSize; }

bool sanityCheck(const protocol::MarketDataMsg &msg) {
  return msg.bid_price >= 0 && msg.ask_price &&
         msg.bid_price <= msg.ask_price && msg.bid_size > 0 && msg.ask_size > 0;
}
